'use strict'
var Field = function(fieldX,fieldY, tileWidth, tileHeight, tileSpace){
    console.log('field created');

    //===================================
    // VARIABLES
    //===================================
    this.GenerateField(fieldX,fieldY,tileWidth, tileHeight, tileSpace);
}
...
Field.prototype.Click = function(event){
    for (var i = 0, l = this.fieldX; i < l; i++) {
        for (var j = 0, l2 = this.fieldY; j < l2; j++) {
            this.tile[i][j].Click(event);
        }
    }
}

Field.prototype.GenerateField = function(fieldX, fieldY, w, h , tileSpace){
    this.fieldX     = fieldX;
    this.fieldY     = fieldY;
    ...
    for (var i = 0, l = fieldX; i < l; i++) {
        ...
        for (var j = 0, l2 = fieldY; j < l2; j++) {
            this.tile[i][j] = new Tile(i*(w+tileSpace), j*(h+tileSpace), w, h, i, j, this);
        }
    }
...
Field.prototype.ShowTile = function(tile){
    if (tile.GetState() != tileState.type.SHOWN && tile.GetSurrounding() === 0 && tile.GetBomb() === false){
...
Field.prototype.GetWidth = function(){
    return this.fieldX*(this.tileWidth+this.tileSpace) - this.tileSpace;
}

Field.prototype.GetHeight = function(){
    return this.fieldY*(this.tileHeight+this.tileSpace) - this.tileSpace;
}
